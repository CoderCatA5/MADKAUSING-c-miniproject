...
int main(void)
{
    ...
    //---------------------------------------------------------
    InitWindow(screenWidth, screenHeight, "MadKauSing");
...
void UpdateGame(void)
{
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            ...
            // Collision logic: ball vs player giving speed in x direction
...
void DrawGame(void)
{
    BeginDrawing();
        ...
        {
            // Draw player bar
            DrawRectangle(player.position.x - player.size.x/2, player.position.y - player.size.y/2, player.size.x, player.size.y, RED);
